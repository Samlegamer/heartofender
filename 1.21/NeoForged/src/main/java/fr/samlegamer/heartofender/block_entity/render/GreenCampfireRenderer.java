package fr.samlegamer.heartofender.block_entity.render;
...
@OnlyIn(Dist.CLIENT)
public class GreenCampfireRenderer implements BlockEntityRenderer<GreenCampfireBlockEntity> {
    private static final float SIZE = 0.375F;
    private final ItemRenderer itemRenderer;

    public GreenCampfireRenderer(BlockEntityRendererProvider.Context pContext) {
        this.itemRenderer = pContext.getItemRenderer();
    }

    public void render(GreenCampfireBlockEntity pBlockEntity, float pPartialTick, PoseStack pPoseStack, MultiBufferSource pBuffer, int pPackedLight, int pPackedOverlay) {
        Direction direction = pBlockEntity.getBlockState().getValue(GreenCampfire.FACING);
