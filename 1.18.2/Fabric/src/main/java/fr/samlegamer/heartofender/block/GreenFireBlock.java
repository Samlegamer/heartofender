...
public class GreenFireBlock extends AbstractGreenFireBlock
{
    ...
    public GreenFireBlock(AbstractBlock.Settings settings) {
        ...
        this.shapesByState = ImmutableMap.copyOf(this.stateManager.getStates().stream().filter(state -> state.get(AGE) == 0).collect(Collectors.toMap(Function.identity(), GreenFireBlock::getShapeForState)));
    }
    ...
    public void scheduledTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        boolean bl2;
        world.createAndScheduleBlockTick(pos, this, GreenFireBlock.getFireTickDelay(world.random));
    ...
    private BlockState getStateWithAge(WorldAccess world, BlockPos pos, int age) {
        ...
        if (blockState.isOf(HoeBlocks.GREEN_FIRE)) {
    ...
    public static void registerDefaultFlammables() {
    	GreenFireBlock fireBlock = (GreenFireBlock)HoeBlocks.GREEN_FIRE;
