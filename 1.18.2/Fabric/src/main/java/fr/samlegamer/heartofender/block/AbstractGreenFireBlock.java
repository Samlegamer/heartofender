...
public abstract class AbstractGreenFireBlock extends Block {
    ...
    public AbstractGreenFireBlock(AbstractBlock.Settings settings, float damage) {
        super(settings);
        this.damage = damage;
    }
    ...
    public BlockState getPlacementState(ItemPlacementContext ctx) {
        return AbstractGreenFireBlock.getState(ctx.getWorld(), ctx.getBlockPos());
    }
    ...
    public static BlockState getState(BlockView world, BlockPos pos) {
        ...
        return ((GreenFireBlock)HoeBlocks.GREEN_FIRE).getStateForPosition(world, pos);
    }
    ...
    public void onBlockAdded(BlockState state, World world, BlockPos pos, BlockState oldState, boolean notify) {
        ...
        if (AbstractGreenFireBlock.isOverworldOrNether(world) && (optional = AreaHelper.getNewPortal(world, pos, Direction.Axis.X)).isPresent()) {
    ...
    public static boolean canPlaceAt(World world, BlockPos pos, Direction direction) {
        ...
        return AbstractGreenFireBlock.getState(world, pos).canPlaceAt(world, pos) || AbstractGreenFireBlock.shouldLightPortalAt(world, pos, direction);
    }

    private static boolean shouldLightPortalAt(World world, BlockPos pos, Direction direction) {
        if (!AbstractGreenFireBlock.isOverworldOrNether(world)) {
            return false;
        }
        BlockPos.Mutable mutable = pos.mutableCopy();
        boolean bl = false;
        for (Direction direction2 : Direction.values()) {
            if (!world.getBlockState(mutable.set(pos).move(direction2)).isOf(HoeBlocks.HEART_ENDER_PORTAL_BLOCK)) continue;
            bl = true;
            break;
        }
        if (!bl) {
