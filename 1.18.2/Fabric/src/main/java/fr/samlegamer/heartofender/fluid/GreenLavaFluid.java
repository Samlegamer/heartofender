...
public abstract class GreenLavaFluid extends FlowableFluid
{
    ...
    public Fluid getFlowing() {
        return HoeFluids.FLOWING_GREEN_LAVA;
    }

    @Override
    public Fluid getStill() {
        return HoeFluids.GREEN_LAVA;
    }

    @Override
    public Item getBucketItem() {
        return HoeItems.GREEN_LAVA_BUCKET;
    }
    ...
    public BlockState toBlockState(FluidState state) {
        return (BlockState)HoeFluids.GREEN_LAVA_BLOCK.getDefaultState().with(FluidBlock.LEVEL, GreenLavaFluid.getBlockStateLevel(state));
    }

    @Override
    public boolean matchesType(Fluid fluid) {
        return fluid == HoeFluids.GREEN_LAVA || fluid == HoeFluids.FLOWING_GREEN_LAVA;
    }
        ...
        if (direction == Direction.DOWN) {
            ...
            if (this.isIn(FluidTags.LAVA) && fluidState2.isIn(FluidTags.WATER)) {
                if (state.getBlock() instanceof FluidBlock) {
                    world.setBlockState(pos, HoeBlocks.DARK_END_STONE.getDefaultState(), Block.NOTIFY_ALL);
                }
                this.playExtinguishEvent(world, pos);
                return;
            }
