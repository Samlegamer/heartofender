...
import fr.samlegamer.heartofender.world.biomes.setgreenfire.MapGenGreenFire;
...
import fr.samlegamer.heartofender.world.worldgen.fortress.WorldGenHeartFortress;
import fr.samlegamer.heartofender.world.worldgen.lavagreen.WorldGenGreenLava;
import fr.samlegamer.heartofender.world.worldgen.setpurpleglowstone.MapGenPurpleGlowstone1;
import fr.samlegamer.heartofender.world.worldgen.setpurpleglowstone.MapGenPurpleGlowstone2;
...
public class ChunkGeneratorHeartofEnder implements IChunkGenerator
{
    protected static final IBlockState AIR = Blocks.AIR.getDefaultState();
    protected static final IBlockState DARK_END_STONE = BlocksMod.dark_end_stone.getDefaultState();
    protected static final IBlockState BEDROCK = Blocks.BEDROCK.getDefaultState();
    protected static final IBlockState GREEN_LAVA = ModBlocks.GREEN_LAVA_BLOCK.getDefaultState();
    protected static final IBlockState DEAD_SAND = BlocksMod.dead_sand.getDefaultState();
    ...
    private final MapGenGreenFire fireFeature = new MapGenGreenFire();
    private final MapGenPurpleGlowstone1 lightGemGen = new MapGenPurpleGlowstone1();
    private final MapGenPurpleGlowstone2 hellPortalGen = new MapGenPurpleGlowstone2();
    private final WorldGenerator quartzGen = new WorldGenMinable(BlocksMod.milathium_ore.getDefaultState(), HoeConfig.milathium_ore_density, BlockMatcher.forBlock(BlocksMod.dark_end_stone));
    private final WorldGenerator magmaGen = new WorldGenMinable(BlocksMod.blue_magma_block.getDefaultState(), HoeConfig.blue_magma_density, BlockMatcher.forBlock(BlocksMod.dark_end_stone));
    private final WorldGenGreenLava lavaTrapGen = new WorldGenGreenLava(ModBlocks.GREEN_LAVA_BLOCK, true);
    private final WorldGenGreenLava hellSpringGen = new WorldGenGreenLava(ModBlocks.GREEN_LAVA_BLOCK, false);
    ...
    //private MapGeneratorHeartFortress genHeartEnderFortress = new MapGeneratorHeartFortress();
    ...
    public ChunkGeneratorHeartofEnder(World worldIn, boolean p_i45637_2_, long seed)
    {
        this.world = worldIn;
        ...
        //this.genHeartEnderFortress = (MapGeneratorHeartFortress)net.minecraftforge.event.terraingen.TerrainGen.getModdedMapGen(genHeartEnderFortress, net.minecraftforge.event.terraingen.InitMapGenEvent.EventType.NETHER_BRIDGE);
                    ...
                    for (int i2 = 0; i2 < 8; ++i2)
                    {
                        ...
                        for (int j2 = 0; j2 < 4; ++j2)
                        {
                            ...
                            for (int k2 = 0; k2 < 4; ++k2)
                            {
                                ...
                                if (l1 * 8 + i2 < j)
                                {
                                    iblockstate = GREEN_LAVA;
                                }

                                if (d15 > 0.0D)
                                {
                                    iblockstate = DARK_END_STONE;
                                }
    ...
    public void buildSurfaces(int p_185937_1_, int p_185937_2_, ChunkPrimer primer)
    {
        ...
        for (int j = 0; j < 16; ++j)
        {
            for (int k = 0; k < 16; ++k)
            {
                ...
                int i1 = -1;
                IBlockState iblockstate = DARK_END_STONE;
                IBlockState iblockstate1 = DARK_END_STONE;
                ...
                for (int j1 = 127; j1 >= 0; --j1)
                {
                    if (j1 < 127 - this.rand.nextInt(5) && j1 > this.rand.nextInt(5))
                    {
                        ...
                        {
                            if (iblockstate2.getBlock() == BlocksMod.dark_end_stone)
                            {
                                if (i1 == -1)
                                {
                                    if (l <= 0)
                                    ...
                                    else if (j1 >= i - 4 && j1 <= i + 1)
                                    {
                                        ...
                                        if (flag)
                                        {
                                            iblockstate = DEAD_SAND;
                                            iblockstate1 = DEAD_SAND;
                                        }
                                    }
    ...
    public Chunk generateChunk(int x, int z)
    {
        ...
        {
            this.genHeartEnderFortress.generate(this.world, x, z, chunkprimer);
        }  */      
    ...
    public void populate(int x, int z)
    {
      ...
      //  this.genHeartEnderFortress.generateStructure(this.world, this.rand, chunkpos);
    ...
    public List<Biome.SpawnListEntry> getPossibleCreatures(EnumCreatureType creatureType, BlockPos pos)
    {
        ...
        {
            if (this.genHeartEnderFortress.isInsideStructure(pos))
            {
                return this.genHeartEnderFortress.getSpawnList();
            }

            if (this.genHeartEnderFortress.isPositionInStructure(this.world, pos) && this.world.getBlockState(pos.down()).getBlock() == BlocksMod.dark_end_stone_brick)
            {
                return this.genHeartEnderFortress.getSpawnList();
            }
        }
    ...
    {
        return "HeartFortress".equals(structureName) && this.genHeartEnderFortress != null ? this.genHeartEnderFortress.getNearestStructurePos(worldIn, position, findUnexplored) : null;
    }

    public boolean isInsideStructure(World worldIn, String structureName, BlockPos pos)
    {
        return "HeartFortress".equals(structureName) && this.genHeartEnderFortress != null ? this.genHeartEnderFortress.isInsideStructure(pos) : false;
    }
    
    public void recreateStructures(Chunk chunkIn, int x, int z)
    {
        this.genHeartEnderFortress.generate(this.world, x, z, (ChunkPrimer)null);
    }*/
