package fr.samlegamer.heartofender.tile;
...
public class GreenCampfireTileEntity extends TileEntity implements IClearable, ITickableTileEntity {
	   private final NonNullList<ItemStack> items = NonNullList.withSize(4, ItemStack.EMPTY);
	   private final int[] cookingProgress = new int[4];
	   private final int[] cookingTime = new int[4];

	   public GreenCampfireTileEntity()
	   {
	      super(ModTileEntities.GREEN_CAMPFIRE_TILE.get());
	   }

	   public void tick() {
	      boolean flag = this.getBlockState().getValue(GreenCampfireBlock.LIT);
	      ...
	      if (world != null) {
	         ...
	         if (random.nextFloat() < 0.11F) {
	            for(int i = 0; i < random.nextInt(2) + 2; ++i) {
	            	GreenCampfireBlock.makeParticles(world, blockpos, this.getBlockState().getValue(GreenCampfireBlock.SIGNAL_FIRE), false);
	            }
	         }

	         int l = this.getBlockState().getValue(GreenCampfireBlock.FACING).get2DDataValue();
